...
#     with open(full_path('data.csv'), 'r', newline='', encoding='utf-8') as f:
...
#             if len(row) > 0 and row[0] == first_name and len(row) > 3:
#                 country = row[3]
#                 break
#         if country == "":
#             print("Specified user data is not available. Tip: Generate random country.")
#     else:
#         filteredData = [row[12] for row in data_rows if len(row) > 12 and row[12].strip()]
#         if not filteredData:
#             print("No country data available in column 12. Using default value.")
#             return "Unknown"
#         return choice(filteredData)
#
#     return country
...
def get_country(first_name=None):
    ...
    if first_name:
        for row in data_rows:
            if len(row) > 0 and row[0] == first_name and len(row) > 3 and row[3].strip():
                return row[3]
        print("Specified user data is not available. Tip: Generate random country.")

    filteredData = [row[12] for row in data_rows if len(row) > 12 and row[12].strip()]
    if not filteredData:
        print("No country data available. Using default.")
        return "Unknown"
